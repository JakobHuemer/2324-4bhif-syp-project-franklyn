...
----
* *Additional Notes*: A valid call to the video generation job will always return a job with status "ONGOING". The returned status code will be 202 (`ACCEPTED`) and the `Location header` will contain a full url to the job status endpoint for the newly created job
