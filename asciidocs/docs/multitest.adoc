...
----
See also: https://github.com/2324-4bhif-syp/2324-4bhif-syp-project-franklyn/blob/feat/multi-client-capabilities/server/src/main/java/at/htl/franklyn/server/feature/exam/ExamDto.java[ExamDto]
...
----
See also: https://github.com/2324-4bhif-syp/2324-4bhif-syp-project-franklyn/blob/feat/multi-client-capabilities/server/src/main/java/at/htl/franklyn/server/feature/exam/ExamInfoDto.java[ExamInfoDto]
...
----
See also https://github.com/2324-4bhif-syp/2324-4bhif-syp-project-franklyn/blob/feat/multi-client-capabilities/server/src/main/java/at/htl/franklyn/server/feature/examinee/ExamineeDto.java[ExamineeDto]
* *Additional Notes*: N/A

====== Get video jobs of exam
* *Path*: `/exams/{id}/videojobs`
* *Method*: `GET`
* *Description*: Get all video jobs which belong to the given exam
* *Request format*: `Path param`
* *Request structure*: `/exams/{id}/videojobs` where `{id}` is a valid numeric ID of an exam
* *Response format*: `JSON`
* *Response structure*:
+
[source, json]
...
[
    {
      "job_id": number,
      "examinee_id": number | null,
      "exam_id": number,
      "status": "QUEUED", "ONGOING" | "DONE" | "FAILED" | "DELETED",
    },
    ...
]
----
...
----
* *Additional Notes*: A valid call to the video generation job will always return a status code of 202 (`ACCEPTED`) and the `Location header` will contain a full url to the job status endpoint for the newly created job
...
----
{
  "type": "CAPTURE_SCREEN" | "DISCONNECT",
  "payload": object | null
}
----
...
====== Disconnect
* *Description*: Requests the student-client (openbox) to disconnect from the server, effectively stopping any telemetry capture at the end of the exam.
* *Command structure*:
+
[source, json]
----
...
----
* *Expected Response*: Nothing. Studentclient (openbox) should disconnect
* *Additional Notes*: For client implementations: Displaying a popup or another attention catching GUI Element to the user is encouraged.
...
* *Path*: `/metrics`
* *Method*: `GET`
* *Description*: Provides information about the server system itself. Including: CPU usage, total disk space available, remaining disk space, size of screenshots and videos, used and available memory
* *Request format*: N/A
* *Request structure*: N/A
* *Response format*: `JSON`
* *Response structure*:
